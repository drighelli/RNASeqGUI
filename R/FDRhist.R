FDRhist <- function(results_EdgeR, the.file, Project){

  #results_EdgeR <- read.table("results_EdgeR.txt", header=TRUE, row.names=1)

  db.cache <- InitDb(db.name='fdrhist_db', db.path=file.path("RNASeqGUI_Projects",Project,"Logs","cache"))
  ...
  hist(results_EdgeR$FDR, breaks=100, col="green", border="slateblue", main="FDR Histogram", xlab="FDR", ylab="Frequency")
...
#write into the project report
     ...
     message2 <- paste(" * In the *Result Inspection Interface*, you clicked the **FDR Hist** button for EdgeR at `", Sys.time(),"` and the ",outputName," file has been saved in the `", Project,"\\Plots` folder.", sep="")
...
message5 <- paste("db.cache <- InitDb(db.name='fdrhist_db', db.path='cache')", sep="")
write(message5, file = report,ncolumns = if(is.character(message5)) 1 else 5,append = TRUE, sep = "\n")
...
message5 <- paste("hist(results_EdgeR$FDR, breaks=100, col='green', border='slateblue', ",sep="")
write(message5, file = report,ncolumns = if(is.character(message5)) 1 else 5,append = TRUE, sep = "\n")
