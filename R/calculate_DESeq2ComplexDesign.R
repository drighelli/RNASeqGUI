calculate_DESeq2ComplexDesign <- function(project_name) {
...
performStatistics_DESeq2ComplexDesign <- function(button, user.data) {
		res <- NULL
		d <- NULL
		error <- NULL
		warning <- NULL
  ...
  design.file <- filename2$getText()
		design=read.table(design.file,header=T,sep="\t",row.names=NULL)   ###### 	LOAD THE FILE #########
  conditions <- design[1,]
  ...
  libTypes <- c(as.character(unlist(libTypes))) 
   ...
   res = NEWDESeq2ComplexDesign(d,conditions,libTypes,the.file,padj,Project) 
		...
		if (!is.null(res)) {
			########## Start dialog...
			# Open a dialog box to print results
			dialog <- gtkDialogNewWithButtons("DESeq2ComplexDesign finished!",window, "modal","gtk-ok", GtkResponseType["ok"],"gtk-quit", GtkResponseType["cancel"])
...
perform_showresults <- function(button, user.data) {
		res <- NULL
  res = DESeq2ShowresultsComplexDesign(Project) # MAIN FUNCTION HERE
  res
  rm(list = ls())
	} #End of function
...
# Create window
	...
	# Add a frame
 title=paste("       DESeq2 for Complex Design Interface is ready to work on ",Project, " project.", sep="")
