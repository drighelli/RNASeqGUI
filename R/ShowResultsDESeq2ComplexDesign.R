DESeq2ShowresultsComplexDesign <- function(Project){
  ...
  if(Sys.info()[[1]]=="Windows"){
      ...
      result_file <- list.files(folder,pattern="_results_DESeq2ComplexDesign.txt$",full.names=TRUE)
      htmlRep <- HTMLReport(shortName = result_file)
      res.table <- read.table(paste(".\\",result_file,sep=""),header=TRUE,row.names=1)
