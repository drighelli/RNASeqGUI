VolcanoDESeq <- function(name,p,results_DESeq,the.file, Project){
...
db.cache <- InitDb(paste(the.file2,db.name='_volcanodeseq_db',sep=''), db.path=file.path("RNASeqGUI_Projects",Project,"Logs","cache"))
SaveInCache(db.cache, the.file, "thefile_key")
SaveInCache(db.cache, Project, "project_key")
SaveInCache(db.cache, results_DESeq, "res_key")
SaveInCache(db.cache, name, "name_key")
SaveInCache(db.cache, p, "p_key")

 plot(results_DESeq$log2FoldChange, -log10(results_DESeq$padj), col = "black", main="DESeq Volcano Plot", xlab="log2FoldChange", ylab="-log10(padj)", pch=19,cex=0.3)

 DE_genes_DESeq = subset(results_DESeq, padj<p)

 points(DE_genes_DESeq$log2FoldChange, -log10(DE_genes_DESeq$padj), pch=19, col="red", cex=0.5)

 if (name!=""){

   OneGene = subset(results_DESeq, row.names(results_DESeq)==name)
 ...
 outputName=paste(the.file2,"_Volcano_DESeq.pdf", sep="")
...
#write into the project report
  report=paste("RNASeqGUI_Projects\\",Project,"\\Logs\\report.Rmd",sep="")
     ...
     message2 <- paste(" * In the *Result Inspection Interface*, you clicked the **Volcano Plot** button for DESeq at `", Sys.time(),"` and the ",outputName," file has been saved in the `", Project,"\\Plots` folder.", sep="")
...
message5 <- paste("db.cache <- InitDb(paste(the.file2,db.name='_volcanodeseq_db',sep=''), db.path='cache')", sep="")
write(message5, file = report,ncolumns = if(is.character(message5)) 1 else 5,append = TRUE, sep = "\n")
message5 <- paste(" results_DESeq <- LoadCachedObject(db.cache, 'res_key')", sep="")
write(message5, file = report,ncolumns = if(is.character(message5)) 1 else 5,append = TRUE, sep = "\n")
...
message5 <- paste("print(head(results_DESeq))",sep="")
write(message5, file = report,ncolumns = if(is.character(message5)) 1 else 5,append = TRUE, sep = "\n")
...
message5 <- paste("plot(results_DESeq$log2FoldChange, -log10(results_DESeq$padj), col = 'black', main='DESeq Volcano Plot', ",sep="")
write(message5, file = report,ncolumns = if(is.character(message5)) 1 else 5,append = TRUE, sep = "\n")

message5 <- paste("xlab='log2FoldChange', ylab='-log10(padj)', pch=19,cex=0.3)",sep="")
write(message5, file = report,ncolumns = if(is.character(message5)) 1 else 5,append = TRUE, sep = "\n")

message5 <- paste("DE_genes_DESeq = subset(results_DESeq, padj<p)",sep="")
write(message5, file = report,ncolumns = if(is.character(message5)) 1 else 5,append = TRUE, sep = "\n")

message5 <- paste("points(DE_genes_DESeq$log2FoldChange, -log10(DE_genes_DESeq$padj), pch=19, col='red', cex=0.5)",sep="")
write(message5, file = report,ncolumns = if(is.character(message5)) 1 else 5,append = TRUE, sep = "\n")

message5 <- paste("if (name!=''){   OneGene = subset(results_DESeq, row.names(results_DESeq)==name)",sep="")
write(message5, file = report,ncolumns = if(is.character(message5)) 1 else 5,append = TRUE, sep = "\n")
...
SaveInCache(db.cache, p, "p_key")
 ...
 plot(results_DESeq$log2FoldChange, -log10(results_DESeq$padj), col = "black", main="DESeq Volcano Plot", xlab="log2FoldChange", ylab="-log10(padj)", pch=16,cex=0.4)
