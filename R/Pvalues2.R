...
function(res,the.file, Project){
  ...
  db.cache <- InitDb(db.name='pvalues2_db', db.path=file.path("RNASeqGUI_Projects",Project,"Logs","cache"))
...
message5 <- paste("db.cache <- InitDb(db.name='pvalues2_db', db.path='cache')", sep="")
write(message5, file = report,ncolumns = if(is.character(message5)) 1 else 5,append = TRUE, sep = "\n")
