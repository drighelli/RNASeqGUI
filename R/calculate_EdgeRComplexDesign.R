calculate_EdgeRComplexDesign <- function(project_name) {
...
infoFun <- function(button, user.data) {
         ...
         message=paste("Vignette",sep="")
        	...
        	dialog$getContentArea()$packStart(vbox, FALSE, FALSE, 0)
	    	  	...
	    	  	dialog$getContentArea()$packStart(vbox, FALSE, FALSE, 0)
    label1 <- gtkLabelNewWithMnemonic(paste0("Suppose you have two treatments (T1, T2) and one control (U) and two replicates for each condition.\n",
   ...
   label10 <- gtkLabelNewWithMnemonic("				U.1			U.2			T1.1			T1.2			T2.1			T2.2")
   label11 <- gtkLabelNewWithMnemonic("				U				U				T1				T1				T2				T2")
   label12 <- gtkLabelNewWithMnemonic("\t\tsingle-end\t\tpaired-end\tpaired-end\tsingle-end\t\tpaired-end\tsingle-end")
   label13 <- gtkLabelNewWithMnemonic("-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------")
   label14 <- gtkLabelNewWithMnemonic("You need to specify the 'Coefficient?' field.")
   label15 <- gtkLabelNewWithMnemonic("Set Coefficient?: 2 , to compare T1 vs U.")
   label16 <- gtkLabelNewWithMnemonic("Set Coefficient?: 3 , to compare T2 vs U.")
   label17 <- gtkLabelNewWithMnemonic("Coefficient?: 1 , should not be used.")
   label18 <- gtkLabelNewWithMnemonic("Finally, click on the 'Run EdgeR GLM for Multi Factors' button.")
   label19 <- gtkLabelNewWithMnemonic("For further information, see www.bioconductor.org/packages/release/bioc/vignettes/edgeR/inst/doc/edgeR.pdf")
 ...
 performStatistics_EdgeRComplexDesign <- function(button, user.data) {
		res <- NULL
		d <- NULL
		error <- NULL
		warning <- NULL
  ...
  d <- read.table(the.file,sep=the.sep,header=the.headers,row.names=1)
		...
		if (sum(numVar)==0) {
			...
			# Run 'EdgeR' 
   res = EdgeRComplexDesign(d,ncol(d),b,the.file,FDR,libTypes,Project,coe) 
...
perform_showresults <- function(button, user.data) {
		res <- NULL
  res = EdgeRShowresultsComplexDesign(Project) # MAIN FUNCTION HERE
  res
  rm(list = ls())
	} #End of function
...
# Create window
	...
	# Add a frame
 title=paste("       EdgeR GLM for Multi Factors Interface is ready to work on ",Project, " project.", sep="")
...
# Add horizontal container for every widget line
 ...
 # Coefficient?
 label <- gtkLabelNewWithMnemonic("_Coefficient?")
	hbox$packStart(label,FALSE,FALSE,0)
	coef <- gtkEntryNew()
	coef$setWidthChars(2)
	coef$setText("2")
	hbox$packStart(coef,FALSE,FALSE,0)
	label$setMnemonicWidget(coef)
...
# Add horizontal container for every widget line
 ...
 # button
	EdgeRComplexDesign <- gtkButtonNewWithMnemonic("Run EdgeR GLM for Multi Factors", show = TRUE)
	gSignalConnect(EdgeRComplexDesign, "clicked", performStatistics_EdgeRComplexDesign)
	the.buttons$packStart(EdgeRComplexDesign,fill=FALSE)
