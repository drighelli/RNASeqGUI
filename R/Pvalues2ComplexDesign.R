Pvalues2ComplexDesign <- function(res,the.file, Project){
  db.cache <- InitDb(db.name='pvaluescomplex_db', db.path=file.path("RNASeqGUI_Projects",Project,"Logs","cache"))
  
  SaveInCache(db.cache, the.file, "thefile_key")
  SaveInCache(db.cache, Project, "project_key")
  SaveInCache(db.cache, res, "res_key")
  #res <- read.table("results_DESeq2.txt", header=TRUE, row.names=1)
  ...
  outputName=paste(the.file2,"_Pvalue_Distribution_DESeq2.pdf", sep="")

  hist(res$pvalue, breaks=100, col="red", border="slateblue",  main="Pvalue Distribution DESeq2", xlab="PValue", ylab="Frequency") #Histogram of pvalues
...
#write into the project report
     ...
     message2 <- paste(" * In the *Result Inspection Interface*, you clicked the **P-value Hist** button for DESeq2 at `", Sys.time(),"` and the ",outputName," file has been saved in the `", Project,"\\Plots` folder.", sep="")
...
message5 <- paste("db.cache <- InitDb(db.name='pvaluescomplex_db', db.path='cache')", sep="")
write(message5, file = report,ncolumns = if(is.character(message5)) 1 else 5,append = TRUE, sep = "\n")
...
message5 <- paste("outputName=paste(the.file2,'_Pvalue_Distribution_DESeq2.pdf', sep='')",sep="")
write(message5, file = report,ncolumns = if(is.character(message5)) 1 else 5,append = TRUE, sep = "\n")

message5 <- paste("hist(res$pvalue, breaks=100, col='red', border='slateblue',  main='Pvalue Distribution DESeq2',",sep="")
write(message5, file = report,ncolumns = if(is.character(message5)) 1 else 5,append = TRUE, sep = "\n")

message5 <- paste("xlab='PValue', ylab='Frequency') #Histogram of pvalues",sep="")
write(message5, file = report,ncolumns = if(is.character(message5)) 1 else 5,append = TRUE, sep = "\n")
