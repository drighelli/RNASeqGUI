calculate_DESeq2 <- function(project_name) {
...
"Finally, click on the 'Run DESeq2' button.\n",
"For further information, see www.bioconductor.org/packages/release/bioc/vignettes/DESeq2/inst/doc/DESeq2.pdf."))
 ...
 performStatistics_DESeq2 <- function(button, user.data) {
		res <- NULL
		d <- NULL
		error <- NULL
		warning <- NULL
  ...
  padj = padj$getText()
   ...
   res = NEWDESeq2(d,conditions,libTypes,treated,control,the.file,padj,Project) 
		...
		if (!is.null(res)) {
			########## Start dialog...
			# Open a dialog box to print results
			dialog <- gtkDialogNewWithButtons("DESeq2 finished!",window, "modal","gtk-ok", GtkResponseType["ok"],"gtk-quit", GtkResponseType["cancel"])
				hbox <- gtkHBoxNew(FALSE,20)
				vbox$packStart(hbox, FALSE, FALSE, 0)
				#label <- gtkLabelNew(paste("Plots are saved in the folder Plots: ",save.dir,exportName,".csv",sep=""))
...
perform_showresults <- function(button, user.data) {
		res <- NULL
  res = DESeq2Showresults(Project) # MAIN FUNCTION HERE
  res
  rm(list = ls())
	} #End of function
...
# Create window
	...
	# Add a frame
 title=paste("       DESeq2 Interface is ready to work on ",Project, " project.", sep="")
...
# Add horizontal container for every widget line
 ...
 # button
	deseq2 <- gtkButtonNewWithMnemonic("Run DESeq2", show = TRUE)
	gSignalConnect(deseq2, "clicked", performStatistics_DESeq2)
	the.buttons$packStart(deseq2,fill=FALSE)

 # button
	#DESeq2ComplexDesign <- gtkButtonNewWithMnemonic("Run DESeq2ComplexDesign", show = TRUE)
	#gSignalConnect(DESeq2ComplexDesign, "clicked", performStatistics_DESeq2ComplexDesign)
